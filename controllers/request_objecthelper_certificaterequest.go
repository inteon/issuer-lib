...
func (c *certificateRequestObjectHelper) RequestObject() signer.RequestObject {
	return signer.RequestObjectFromCertificateRequest(c.readOnlyObj)
}
...
func (c *certificateRequestPatchHelper) SetInitializing() bool {
	...
	if ready := conditions.GetCertificateRequestStatusCondition(
		c.readOnlyObj.Status.Conditions,
		cmapi.CertificateRequestConditionReady,
	); ready != nil {
		return false
...
func (c *certificateRequestPatchHelper) SetWaitingForIssuerReadyNoCondition() {
	message, _ := c.setCondition(
		...
		cmmeta.ConditionFalse,
		cmapi.CertificateRequestReasonPending,
		"Issuer is not Ready yet. No ready condition found. Waiting for it to become ready.",
	)
...
func (c *certificateRequestPatchHelper) SetWaitingForIssuerReadyNotReady(cond *cmapi.IssuerCondition) {
	message, _ := c.setCondition(
		...
		cmmeta.ConditionFalse,
		cmapi.CertificateRequestReasonPending,
		fmt.Sprintf("Issuer is not Ready yet. Current ready condition is \"%s\": %s. Waiting for it to become ready.", cond.Reason, cond.Message),
	)
	c.eventRecorder.Event(c.readOnlyObj, corev1.EventTypeNormal, eventWaitingForIssuerReady, message)
}
...
func (c *certificateRequestPatchHelper) SetCustomCondition(
	...
	currentCustom := conditions.GetCertificateRequestStatusCondition(c.readOnlyObj.Status.Conditions, cmapi.CertificateRequestConditionType(conditionType))
