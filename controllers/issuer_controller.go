...
func (r *IssuerReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	issuerStatusPatch, returnedError := r.reconcileStatusPatch(logger, ctx, req)
	logger.V(2).Info("Got StatusPatch result", "patch", issuerStatusPatch, "error", returnedError)
...
func (r *IssuerReconciler) reconcileStatusPatch(
	...
	ctx context.Context,
	req ctrl.Request,
) (issuerStatusPatch *v1alpha1.IssuerStatus, returnedError error) {
	...
	if r.IgnoreIssuer != nil {
		...
		if err != nil {
			return nil, fmt.Errorf("failed to check if issuer should be ignored: %v", err) // requeue with backoff
		}
		if ignore {
	...
	if readyCondition == nil {
		...
		return issuerStatusPatch, nil // apply patch, done
	}
	...
	if err == nil {
		...
		r.EventRecorder.Eventf(issuer, corev1.EventTypeNormal, eventChecked, message)
	...
	if isPermanentError {
		...
		r.EventRecorder.Event(issuer, corev1.EventTypeWarning, eventPermanentError, message)
		return issuerStatusPatch, reconcile.TerminalError(err) // apply patch, done
	} else {
		// retry
		...
		r.EventRecorder.Eventf(issuer, corev1.EventTypeWarning, eventRetryableError, message)
		return issuerStatusPatch, err // apply patch, requeue with backoff
	}
}
